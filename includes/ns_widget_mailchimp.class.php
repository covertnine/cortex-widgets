...
class NS_Widget_MailChimp extends WP_Widget
{
        ...
        if (isset($_GET[$this->id_base . '_email'])) {
            ...
            if (! is_email($_GET[$this->id_base . '_email'])) { //Use WordPress's built-in is_email function to validate input.
                ...
                if (false == $this->ns_mc_plugin) {
                    ...
                    // Where the Mailchimp V3 API upgrades are happening
                    $mcapi_three = new MailChimp($mcapi->api_key);
                    $list_id = $this->get_current_mailing_list_id($_GET['ns_mc_number']);
                    $subscribed = $mcapi_three->post(
                        "lists/$list_id/members",
                      [
                      'email_address' => $_GET[$this->id_base . '_email'],
                      'status' => 'subscribed',
    ...
    // This function isn't is use right now but could be helpful in future
    public function buildParamArray($id, $email_address, $merge_vars=null, $email_type='html', $double_optin=true, $update_existing=false, $replace_interests=true, $send_welcome=false)
    {
        $params = array();
        $params["id"] = $id;
    ...
    public function widget($args, $instance)
    {
        ...
        if ((isset($_COOKIE[$this->id_base . '-' . $this->number]) && $this->hash_mailing_list_id($this->number) == $_COOKIE[$this->id_base . '-' . $this->number]) || false == $this->ns_mc_plugin->get_mcapi()) {
            ...
            if ($this->successful_signup) {
                ...
                if ($template == '') {
                    $template = CORTEX_WIDGETS_PATH . '/widget-templates/widget-mailchimp-template.php';
                }
