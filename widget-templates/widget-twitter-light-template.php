...
if( !empty( $tweets['tweets'] ) and empty( $tweets['tweets']->errors ) ) {
	...
	foreach( $tweets['tweets'] as $tweet ) {
		if( is_object( $tweet ) ) {
			...
			$tweet_text = preg_replace("/(http:|https:)(\/\/|(www\.))(([^\s<]{4,68})[^\s<]*)/", '<a href="$1$2$4" target="_blank" class="dark-color-text">$1$2$4</a>', $tweet_text);

		?>
		<div class="twitter-tweet <?php if ($instance['style'] == 'lightlarge') { echo "big ";} ?>light">
			<div class="twitter-container">
					...
					<div class="row">
						...
						<ul>
							<li class="icon-twitter-normal">
								...
								<a class="date clearfix" href="http://twitter.com/<?php echo $user->screen_name; ?>" target="_blank"><span class="dark-color-text"><?php echo human_time_diff( strtotime( $tweet->created_at ) ); ?> <?php _e('ago', 'cortex'); ?></span></a>
							</li>
						</ul>
						<?php
						if (!empty($instance['show_link'])) { //check to see if they want follow link displayed
							echo '<a class="follow-link action-link dark-color-text" href="http://twitter.com/' . $user->screen_name . '" target="_blank">';
				...
				<div class="bg light-color-bg">&nbsp;</div>
